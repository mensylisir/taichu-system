...
func (r *SecurityPolicyRepository) Upsert(policy *model.SecurityPolicy) error {
	// 检查是否已存在该集群的策略
...
func (r *SecurityPolicyRepository) GetByClusterID(clusterID string) (*model.SecurityPolicy, error) {
...
func (r *SecurityPolicyRepository) DeleteByClusterID(clusterID string) error {
	return r.db.Where("cluster_id = ?", clusterID).Delete(&model.SecurityPolicy{}).Error
}
